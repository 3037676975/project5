...
if ROUTES_REGISTERED:
    ...
    async def voice_note_cache_headers(request, call_next):
        ...
        # after the existing preview script so the API-document tab can be replaced
        # without touching the stable console-v2.html deployment surface.
        if request.url.path == "/" and "text/html" in response.headers.get("content-type", ""):
            ...
            if API_DOCS_SCRIPT not in html:
                html = html.replace("</body>", API_DOCS_SCRIPT + "</body>")
            headers = {
                key: value
